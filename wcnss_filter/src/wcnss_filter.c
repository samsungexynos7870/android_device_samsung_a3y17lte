...
#define PROP_FILTER_START   "vendor.wc_transport.start_hci"
 ...
 * decrement, as in the reference implementation.
 */
static void reconcile_ref_count(void) {
    char value[PROPERTY_VALUE_MAX];
    int ref;

    if (property_get(PROP_CLEAN_UP, value, "0") <= 0 || value[0] == '0') {
        if (property_get(PROP_REF_COUNT, value, "0") > 0) {
            ref = atoi(value);
            if (ref > 0) {
                ref--;
                snprintf(value, sizeof(value), "%d", ref);
                if (property_set(PROP_REF_COUNT, value) < 0)
                    ALOGW("%s: failed to write %s=%s", __func__, PROP_REF_COUNT,
                          value);
                else
                    ALOGI("%s: %s -> %d (abnormal client departure)",
                          __func__, PROP_REF_COUNT, ref);
            }
        }
    } else {
        ALOGV("%s: clean_up set, ref_count handled by libbt-vendor", __func__);
    }
}
...
out:
    ...
    property_set(PROP_FILTER_START, "false");
    if (fd_ctrl_cli >= 0)
        close(fd_ctrl_cli);
    if (fd_bt_cli >= 0)
        close(fd_bt_cli);
